module Spec = struct
  ...
  let empty = { name = None; items = [] }
  let of_item item = { name = None; items = [ item ] }
  ...
  let show_item (item : item) =
    let show_option_flag names _doc =
      Format.sprintf
        "[%s]"
        (names |> CCList.map dashed |> CCString.concat " | ")
    in
    let show_option_value names parameter _doc =
      ...
      Format.sprintf
        "[%s]"
        (names |> CCList.map decorate |> CCString.concat " | ")
    in
    let show_positional parameter _doc = Format.sprintf "<%s>" parameter in
    match item with
    | Flag (names, doc) -> show_option_flag names doc
    | FlagParam (names, parameter, doc) ->
        show_option_value names parameter doc
  ...
  let usage (default_name : string) (spec : t) =
    ...
    Printf.sprintf
      "%s %s"
      name
      (spec.items |> CCList.map show_item |> CCString.concat " ")


  let find_flag (spec : t) arg =
    let option_eq opt_name = dashed opt_name = arg in
    let check = function
...
module Parse = struct
  ...
  let rec parse_args (spec : Spec.t) (args : string list) :
      (found_item list, _) result =
    let is_dashed arg = CCString.get arg 0 = '-' in
    match args with
    | arg :: args' when is_dashed arg ->
      ( match
          (Spec.find_flag spec arg, Spec.find_flag_param spec arg, args')
        with
      | None, None, _ -> Error (Printf.sprintf "No such option %s" arg)
      | Some flag, _, _ ->
          parse_args spec args' >>= fun found -> Ok ((flag, "") :: found)
      | None, Some _, [] ->
          Error (Printf.sprintf "Expected parameter for option %s" arg)
      | None, Some flag_param, param :: args' ->
          parse_args spec args'
          >>= fun found -> Ok ((flag_param, param) :: found) )
  ...
  and parse_positional spec args =
    ...
    | [], [] -> Ok []
    | exp :: _, [] ->
        Error (Printf.sprintf "Expected \"%s\" argument" (get_name exp))
    | [], arg :: _ ->
        Error (Printf.sprintf "Unexpected positional argument \"%s\"" arg)
    | exp :: spec', arg :: args' ->
        parse_positional spec' args' >>= fun found -> Ok ((exp, arg) :: found)
end
...
module Eval = struct
  ...
  let apply (f_t : ('a -> 'b) t) (a_t : 'a t) args =
    f_t args >>= fun f -> a_t args >>= fun a -> Ok (f a)


  let pure a _args = Ok a
  ...
  let run (eval : 'a t) (args : Parse.ast) : ('a, string) result = eval args
end
...
let pure a : 'a t = { spec = Spec.empty; eval = Eval.pure a; name = None }
...
let program ?name (main : 'a) : 'a t =
  let name =
    ...
    | x -> x
  in
  { spec = Spec.empty; eval = Eval.pure main; name }


let usage (prog : 'a t) : string =
  let cmd =
    match prog.name with
    | Some n -> n
    | None -> ""
  in
...
let run (prog : unit t) : unit =
  ...
  match
    Parse.parse_args prog.spec args
    >>= fun preproc -> Eval.run prog.eval preproc
  with
  | Ok () -> ()
  | Error msg ->
      Printf.eprintf "%s\n" msg ;
...
let flag ?doc ?(alias = []) name : bool t =
  ...
  { spec = Spec.of_item item; eval = Eval.flag item; name = None }


let flag_param ?doc ?(alias = []) name param : string option t =
  let item = Spec.FlagParam (name :: alias, param, doc) in
  { spec = Spec.of_item item; eval = Eval.flag_param item; name = None }


let positional ?doc param : string t =
  let item = Spec.Positional (param, doc) in
  { spec = Spec.of_item item; eval = Eval.positional item; name = None }
